...
/**
 * ClassName:HttpLogEntityBuilder
 * Package:com.heyqing.disk.web.log
 * Description:
 *
 * @Date:2024/1/6
...
public class HttpLogEntityBuilder {
    ...
    public static HttpLogEntity build(ContentCachingRequestWrapper requestWrapper, ContentCachingResponseWrapper responseWrapper, StopWatch stopWatch){
        ...
        httpLogEntity.setRequestUrl(requestWrapper.getRequestURI())
        ...
        if (requestWrapper.getMethod().equals(RequestMethod.GET.name())){
            httpLogEntity.setRequestParam(JSON.toJSONString(requestWrapper.getParameterMap()));
        }else{
            httpLogEntity.setRequestParam(new String(requestWrapper.getContentAsByteArray()));
        }
